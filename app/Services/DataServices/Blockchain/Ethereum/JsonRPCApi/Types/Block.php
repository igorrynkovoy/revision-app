...
class Block
{
    ...
    public function __construct(array $response)
    {
        ...
        $this->mixHash = isset($response['mixHash']) ? new Hash($response['mixHash']) : null;
        $this->nonce = isset($response['nonce']) ? $response['nonce'] : null;
        ...
        $this->date = Carbon::createFromTimestamp($this->timestamp);
        ...
        foreach ($response['transactions'] as $transaction) {
            $this->transactions[] = is_array($transaction) ? new TransactionInfo($transaction) : new TransactionHash($transaction);
        }
        $this->uncles = [];
