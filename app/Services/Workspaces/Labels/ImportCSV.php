...
namespace App\Services\Workspaces\Labels;
...
class ImportCSV
{
    private const REQUIRED_HEADERS = ['blockchain', 'type', 'key', 'label', 'tag', 'description'];
    ...
    public function save()
    {
        foreach ($this->csvHandler as $line => $record) {
            ...
            try {
                ...
                $this->errors[$line] = ['key' => ['Duplicated']];
            }
        }
    }
    ...
    {
        $validator = \Validator::make($data, Workspace\Label::getValidationRules());
